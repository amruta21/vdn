    ...
    try {
        ...
        while (headerLength > 32 + fields.length * 32) {
            ...
            let field = {
                ...
                type:  iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'STEPCODE' || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'DATE' || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'STEPNUMBER'  || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'N1'  || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'N2' || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'N3' || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'N4' || iconv.decode(buffer.slice(0, 10), 'ISO-8859-1').split('\0')[0] == 'N5' ? 'C' : String.fromCharCode(buffer[0x0B]) == 0 ? 'I': String.fromCharCode(buffer[0x0B]),
...
async function readRecordsFromDBF(dbf, maxCount) {
    ...
    try {
        ...
        while (true) {
            ...
            // Parse each record.
            for (let i = 0, offset = 0, val = 0; i < recordCountToRead; ++i) {
                let record = {};
                        ...
                        switch (field.type) {
                            ...
                            case 'I': // Integer
                                if(offset <= 79496) {
                                    ...
                                    value = buffer.readInt32LE(val + field.size);
                                    val = val + field.size;
                                }
                                break;
                            case 'M': // Memo
